...
/* END */
		Position Find(ElementType* fn, ElementType* sn, HashTable H)//finds based on first name and second name
		{
			Position P;
			List L;
			int check = 0;
...
/* 3*/      while( P != NULL)
		{
				if (strcmp(P->first_name, key) == 0) {
					...
					if (strlen(P->first_name) >= 8)
						printf("%s\t", P->first_name);
					else
						printf("%s\t\t", P->first_name); //Then node is printed 
					if (strlen(P->last_name) >= 8)
		...
		void update(ElementType* fname, HashTable H)
		{
			...
			else 
			{
				...
				else if (choice == 4)
				{
					...
					scanf_s("%s", &P->first_name, constant);
					getchar();
				}
				else if (choice == 2)
				{
